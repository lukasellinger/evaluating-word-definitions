...
class FactScorer(object):
    ...
    def __init__(self,
        ...
        if "llama" in model_name:
            self.lm = CLM("inst-llama-7B", model_dir='meta-llama/Meta-Llama-3-8B-Instruct',
    ...
    def register_knowledge_source(self, name="enwiki-20230401", db_path=None, data_path=None):
        ...
        self.retrieval[name] = Retrieval(self.db[name], cache_path, embed_cache_path,
    ...
    def get_score(self,
        ...
        if type(topics) == type(generations) == str:
            ...
            assert type(topics) == type(
                generations) == list, "`topics` and `generations` should be lists."
            assert len(topics) == len(
                generations), "`topics` and `generations` should have the same length"
        ...
        if atomic_facts is not None:
            ...
            if self.af_generator is None:
                self.af_generator = AtomicFactGenerator(key_path=self.openai_key,
                                                        demon_dir=os.path.join(self.data_dir,
                                                                               "demos"),
            ...
            self.print_cost_estimates(total_words, task="atomic fact generation",
                                      model="davinci-003")
        ...
        if "ChatGPT" in self.model_name:
            ...
            self.print_cost_estimates(total_words, task="factscore evaluation",
                                      model="gpt-3.5-turbo")
    ...
    def _get_score(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None):
        ...
        for atom in atomic_facts:
            ...
            if self.lm:
                ...
                if cost_estimate:
                    if cost_estimate == "consider_cache" and (
                            prompt.strip() + "_0") not in self.lm.cache_dict:
            ...
            if is_supported and "npm" in self.model_name:
                ...
                is_supported = npprob > 0.3  # already supports if > 0.3
...
class CustomFactScorer(FactScorer):
    ...
    def register_knowledge_source(self, name="test", db_path=None, data_path=None):
        assert name not in self.retrieval, f"{name} already registered"
        if db_path is None:
            db_path = os.path.join(self.data_dir, f"{name}.db")
        ...
        if "npm" in self.model_name:
            ...
            embed_cache_path = os.path.join(self.cache_dir, f"bm25-{name}.pkl")
            self.npm[name] = NPM(
                Retrieval(None, cache_path, embed_cache_path, retrieval_type="wiki-api",
                          word_lang='de', only_intro=True),
                "npm-single",
                cache_file=os.path.join(self.cache_dir, f"npm-{name}.pkl"))
    ...
    with open(args.input_path) as f:
        for line in f:
            ...
            if args.use_atomic_facts:
                ...
                atomic_facts.append([atom["text"] for sent in dp["annotations"] for atom in
                                     sent["model-atomic-facts"]])
            else:
                topics.append(dp["topic"])
                generations.append(dp["output"])
