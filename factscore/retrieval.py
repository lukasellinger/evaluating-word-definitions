...
class DocDB(object):
    ...
    def get_text_from_title(self, title):
        ...
        results = [{"title": title, "text": para} for para in
                   results[0][0].split(SPECIAL_SEPARATOR)]
        assert len(
            results) > 0, f"`topic` in your data ({title}) is likely to be not a valid title in the DB."
        return results
...
class Retrieval(object):
    ...
    def __init__(self, db, cache_path, embed_cache_path,
                 retrieval_type="gtr-t5-large", batch_size=None, word_lang=None, only_intro=None):
        self.db = db
        self.cache_path = cache_path
        self.embed_cache_path = embed_cache_path
        self.retrieval_type = retrieval_type
        self.batch_size = batch_size
        assert retrieval_type == "bm25" or retrieval_type.startswith(
            "gtr-") or retrieval_type == 'wiki-api'
    ...
    def get_bm25_passages(self, topic, query, passages, k):
        if topic in self.embed_cache:
            ...
            bm25 = BM25Okapi(
                [psg["text"].replace("<s>", "").replace("</s>", "").split() for psg in passages])
    ...
    def get_passages(self, topic, question, k):
        if self.retrieval_type == 'wiki-api':
            return self.get_wiki_api_passages(topic, question, k)
        else:
    ...
    def get_wiki_api_passages(self, topic, question, k):
        topic, fall_back_topic = topic
        ...
        if cache_key not in self.cache:
            ...
            self.cache[cache_key] = {'passages': passages, 'word': wiki_word}
            assert len(self.cache[cache_key]['passages']) in [k, len(passages)]
            self.add_n += 1
